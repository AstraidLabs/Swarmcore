...
internal static class AdminCapabilities
{
    ...
    [
        ...
        CreateReadCapability("admin.read.swarms", AdminPermissions.NodesView, "swarm", "/api/admin/swarms", "View runtime swarms", "monitoring"),
        CreateWriteCapability("admin.cleanup.swarm", AdminPermissions.MaintenanceExecute, "POST", false, null, null, "single", "non_idempotent", true, AdminReauthenticationSeverities.Medium, false, true, AdminReauthenticationSeverities.Medium, "monitoring", "swarm", "/api/admin/swarms/{infoHash}/cleanup", "Cleanup stale peers", "Sign in again to confirm swarm cleanup.", "empty", null, [], [], new Dictionary<string, string>(StringComparer.Ordinal), "single_snapshot", "swarm", null)
    ];
